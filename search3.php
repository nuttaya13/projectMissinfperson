...
<div class="page-header_wrap">
      ...
      <div class="page-heading">
        <h1>คำค้นหา"<?php echo $_POST['searchdata2'];?>"</h1>
      </div>
    </div>
...
$searchdata2=$_POST['searchdata2'];
$sql = "SELECT tblreport.id from tblreport 
join tblbrands on tblbrands.id=tblreport.Brand 
where tblreport.Fullname=:search || tblreport.Sex=:search || tblbrands.BrandName=:search || tblreport.Nickname=:search";
$query = $dbh -> prepare($sql);
$query -> bindParam(':search',$searchdata2, PDO::PARAM_STR);
$query->execute();
...
<?php $sql = "SELECT tblreport.*,tblbrands.BrandName,tblbrands.id as bid  from tblreport join tblbrands on tblbrands.id=tblreport.Brand
where tblreport.Fullname=:search || tblreport.Sex=:search || tblbrands.BrandName=:search || tblreport.Nickname=:search";
$query = $dbh -> prepare($sql);
$query -> bindParam(':search',$searchdata2, PDO::PARAM_STR);
$query->execute();
